...
{
  while(*angle < -MaypleMath::cPi)
  {
    *angle += MaypleMath::cTwoPi;
  }
  while(*angle > MaypleMath::cPi)
  {
    *angle -= MaypleMath::cTwoPi;
...
{
  float projCoef = MaypleMath::Dot(vector,normal);
...
void ToAxisAngle(QuatParam quaternion, Vec4Ptr axisAngle)
{
  ...
  axisAngle->w = 2.0f * MaypleMath::ArcCos(tempQuat.w);
  float invSinAngle = MaypleMath::Sqrt(1.0f - tempQuat.w * tempQuat.w);

  if(MaypleMath::Abs(invSinAngle) < 0.0005f)
  {
    invSinAngle = 1.0f;
  }
  else
...
void ToAxisAngle(QuatParam quaternion, Vec3Ptr axis, float* radians)
{
  ...
  *radians = 2.0f * MaypleMath::ArcCos(tempQuat.w);
  float invSinAngle = MaypleMath::Sqrt(1.0f - tempQuat.w * tempQuat.w);
...
void ToEulerAngles(Mat3Param matrix, EulerAnglesPtr eulerAngles)
{
  ...
  {
    float sy = MaypleMath::Sqrt(matrix(i, j) * matrix(i, j) + 
                         matrix(i, k) * matrix(i, k));
    if(sy > 16.0f * FLT_EPSILON)
    {
      (*eulerAngles)[cX] = MaypleMath::ArcTan2(matrix(i, j), matrix(i, k));
      (*eulerAngles)[cY] = MaypleMath::ArcTan2(sy, matrix(i, i));
      (*eulerAngles)[cZ] = MaypleMath::ArcTan2(matrix(j, i), -matrix(k, i));
    }
    else
    {
      (*eulerAngles)[cX] = MaypleMath::ArcTan2(-matrix(j, k), matrix(j, j));
      (*eulerAngles)[cY] = MaypleMath::ArcTan2(sy, matrix(i, i));
      (*eulerAngles)[cZ] = 0.0f;
    }
  }
  ...
  {
    float cy = MaypleMath::Sqrt(matrix(i, i) * matrix(i, i) + 
                         matrix(j, i) * matrix(j, i));
    if(cy > 16.0f * FLT_EPSILON)
    {
      (*eulerAngles)[cX] = MaypleMath::ArcTan2(matrix(k, j), matrix(k, k));
      (*eulerAngles)[cY] = MaypleMath::ArcTan2(-matrix(k, i), cy);
      (*eulerAngles)[cZ] = MaypleMath::ArcTan2(matrix(j, i), matrix(i, i));
    }
    else
    {
      (*eulerAngles)[cX] = MaypleMath::ArcTan2(-matrix(j, k), matrix(j, j));
  ...
  {
    MaypleMath::Swap((*eulerAngles)[cX], (*eulerAngles)[cZ]);
  }
}
...
void ToMatrix3(Vec4Param axisAngle, Mat3Ptr matrix)
{
  ...
  float c0 = MaypleMath::Cos(axisAngle.w);
  float n1C0 = 1.0f - c0;
  float s0 = MaypleMath::Sin(axisAngle.w);
...
void ToMatrix3(Vec3Param axis, float radians, Mat3Ptr matrix)
{
  ...
  float c0 = MaypleMath::Cos(radians);
  float n1C0 = 1.0f - c0;
  float s0 = MaypleMath::Sin(radians);
...
void ToMatrix3(EulerAnglesParam eulerAngles, Mat3Ptr matrix)
{
  ...
  if(EulerOrders::Rotated == frame)
  {
    MaypleMath::Swap(angles[cX], angles[cZ]);
  }
  if(EulerOrders::Odd == parity)
  {
    angles[cX] *= -1.0f;
  ...
  c[I] = MaypleMath::Cos(t[I]); c[J] = MaypleMath::Cos(t[J]); c[H] = MaypleMath::Cos(t[H]);
  s[I] = MaypleMath::Sin(t[I]); s[J] = MaypleMath::Sin(t[J]); s[H] = MaypleMath::Sin(t[H]);
...
void ToQuaternion(EulerAnglesParam eulerAngles, QuatPtr quaternion)
{
  ...
  float t[3], c[3], s[3];
  t[I] = angles[cX] * 0.5f; c[I] = MaypleMath::Cos(t[I]); s[I] = MaypleMath::Sin(t[I]);
  t[J] = angles[cY] * 0.5f; c[J] = MaypleMath::Cos(t[J]); s[J] = MaypleMath::Sin(t[J]);
  t[H] = angles[cZ] * 0.5f; c[H] = MaypleMath::Cos(t[H]); s[H] = MaypleMath::Sin(t[H]);
...
void ToQuaternion(Mat3Param matrix, QuatPtr quaternion)
{
  ...
  if(matrix.m00 + matrix.m11 + matrix.m22 > 0.0f)
  {
    ...
    float s = MaypleMath::Rsqrt(t) * 0.5f;
...
void GenerateOrthonormalBasis(Vec3Param w, Vec3Ptr u, Vec3Ptr v)
{
  ...
  if((MaypleMath::Abs(w.x) >= MaypleMath::Abs(w.y)) && (MaypleMath::Abs(w.x) >= MaypleMath::Abs(w.z)))
  {
    u->x = -w.y;
    u->y = w.x;
    u->z = 0.0f;
...
float Angle(QuatParam a, QuatParam b)
{
  ...
  if(dot == 0.0f)
    return 0.0f;
  float angle = 2.0f * MaypleMath::ArcCos(dot);
  return angle;
}
...
Vector2 Slerp(Vec2Param start, Vec2Param end, float t)
{
  ...
  float theta = MaypleMath::ArcCos(dot) * t;
  Vector2 relativeVec = end - start * dot;
  relativeVec.Normalize();
  return (start * MaypleMath::Cos(theta)) + (relativeVec * MaypleMath::Sin(theta));
}
...
type GenericTowards(const type& a, const type& b, float maxAngle)
{
  ...
  if(angle > MaypleMath::cPi)
  {
    angle -= MaypleMath::cTwoPi;
  }

  angle = MaypleMath::Abs(angle);
...
float SignedAngle(Vec3Param a, Vec3Param b, Vec3Param up)
{
  ...
  Vec3 right = MaypleMath::Cross(a, up);
  right.AttemptNormalize();

  // Get the forward and right dot products
  float forwardDot = MaypleMath::Clamp(MaypleMath::Dot(a, b), -1.0f, 1.0f);
  float rightDot = MaypleMath::Clamp(MaypleMath::Dot(right, b), -1.0f, 1.0f);

  // Get the actual angle from the forward dot product
  float finalAngle = MaypleMath::ArcCos(forwardDot);
...
void OrderPointsCCW(Vector4 viewpoint, Vector4 vertex0, Vector4 vertex1, Vector4 vertex2, int& vertIndex0, int& vertIndex1, int& vertIndex2)
{
  Vector4 planeNormal = Cross(vertex1 - vertex0, vertex2 - vertex1);
  if(Dot(planeNormal, vertex0 - viewpoint) <= -EPSILON)
    Swap(vertIndex0, vertIndex2);
}
