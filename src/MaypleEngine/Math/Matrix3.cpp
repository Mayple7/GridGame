...
{
  Matrix3& self = *this;
  ErrorIf(MaypleMath::DebugIsZero(rhs), "Matrix3 - Division by zero.");
  self[0] /= rhs;
  self[1] /= rhs;
  self[2] /= rhs;
}
...
{
  float c0 = MaypleMath::Cos(radian);
  float n1C0 = 1.0f - c0;
  float s0 = MaypleMath::Sin(radian);
...
void Matrix3::BuildTransform(Vec2Param translate, float radians, Vec2Param scale)
{
  ...
  //Rotation
  m00 = MaypleMath::Cos(radians);
  m01 = -MaypleMath::Sin(radians);
  m10 = -m01;
  m11 = m00;
