...
{
  Matrix4& self = *this;
  ErrorIf(MaypleMath::DebugIsZero(rhs), "Matrix4 - Division by zero.");
  self[0] /= rhs;
  self[1] /= rhs;
  self[2] /= rhs;
  self[3] /= rhs;
...
{
  MaypleMath::Swap(m01, m10);
  MaypleMath::Swap(m02, m20);
  MaypleMath::Swap(m03, m30);
  MaypleMath::Swap(m12, m21);
  MaypleMath::Swap(m13, m31);
  MaypleMath::Swap(m23, m32);
  return *this;
}
...
Matrix4 Matrix4::Inverted() const
{
  ...
  ErrorIf(MaypleMath::DebugIsZero(determinant), "Matrix4 - Uninvertible matrix.");
                        ...
                        Vec3Ptr translate) const
{
  ...
  scale->x = MaypleMath::Sqrt(MaypleMath::Sq(m00) + MaypleMath::Sq(m10) + MaypleMath::Sq(m20));
  ...
  scale->y = MaypleMath::Sqrt(MaypleMath::Sq(m01) + MaypleMath::Sq(m11) + MaypleMath::Sq(m21));
  ...
  scale->z = MaypleMath::Sqrt(MaypleMath::Sq(m02) + MaypleMath::Sq(m12) + MaypleMath::Sq(m22));
...
Matrix4 OuterProduct(Vec4Param u, Vec4Param v)
{
  Matrix4 returnMatrix;
  returnMatrix.m00 = u.x * v.x;
  returnMatrix.m01 = u.x * v.y;
  returnMatrix.m02 = u.x * v.z;
  returnMatrix.m03 = u.x * v.w;

  returnMatrix.m10 = u.y * v.x;
  returnMatrix.m11 = u.y * v.y;
  returnMatrix.m12 = u.y * v.z;
  returnMatrix.m13 = u.y * v.w;

  returnMatrix.m20 = u.z * v.x;
  returnMatrix.m21 = u.z * v.y;
  returnMatrix.m22 = u.z * v.z;
  returnMatrix.m23 = u.z * v.w;
