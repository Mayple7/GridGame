...
void Quaternion::operator/=(float rhs)
{
  ErrorIf(MaypleMath::DebugIsZero(rhs), "Quaternion - Division by zero.");
...
float Quaternion::Normalize()
{
  ...
  {
    length = MaypleMath::Rsqrt(length);
    *this *= length;
    return length;
  }
}
...
{
  return MaypleMath::Sqrt(LengthSq());
}
...
void Quaternion::Invert()
{
  ...
  ErrorIf(MaypleMath::DebugIsZero(lengthSq), "Quaternion - Division by zero.");
  *this /= lengthSq;
}
...
Quaternion Quaternion::Exponent() const
{
  ...
  if(MaypleMath::Abs(angle) > MaypleMath::DebugEpsilon())
  {
    quaternion.w = MaypleMath::Cos(angle);
    angle = MaypleMath::Sin(angle) / angle;
    quaternion.x *= angle;
    quaternion.y *= angle;
    quaternion.z *= angle;
  }
...
{
  Quat quaternion(x, y, z, 0.0f);
  float theta = MaypleMath::ArcCos(w);
  float sinTheta = MaypleMath::Sin(theta);

  if(MaypleMath::Abs(sinTheta) > MaypleMath::DebugEpsilon())
  {
...
{
  Quat tempVec = MaypleMath::ToQuaternion(*vector);
...
{
  return MaypleMath::IsValid(x) && MaypleMath::IsValid(y) && 
         MaypleMath::IsValid(z) && MaypleMath::IsValid(w);
}
...
{
  WarnIf(!MaypleMath::InRange(tValue, 0.0f, 1.0f), 
...
Quaternion Slerp(QuatParam start, QuatParam end, float tValue)
{
  ...
  {
    float theta = MaypleMath::ArcCos(cosTheta);
    float sinTheta = MaypleMath::Sin(theta);
    startVal = MaypleMath::Sin((1.0f - tValue) * theta) / sinTheta;
    endVal = MaypleMath::Sin(tValue * theta) / sinTheta;
  }
  else
  {
...
Quaternion CreateDiagonalizer(Mat3Param matrix)
{
  ...
  for(unsigned i = 0; i < cMaxSteps; ++i)
  {
    ...
    float theta = (diagMatrix(k2, k2) - diagMatrix(k1, k1)) / 
                 (2.0f * offDiag[k]);
    float sign = MaypleMath::GetSign(theta);
    
    //Make theta positive
    theta *= sign;

    //Large term in T
    float thetaTerm = theta < 1e6f ? MaypleMath::Sqrt(MaypleMath::Sq(theta) + 1.0f)
                                       : theta;
    ...
    //c = 1 / (t^2 + 1)      t = s / c
    float c = 1.0f / MaypleMath::Sqrt(MaypleMath::Sq(t) + 1.0f);
    ...
    jacobi[k] = sign * MaypleMath::Sqrt((1.0f - c) / 2.0f);

    //Since our quat-to-matrix convention was for v*M instead of M*v
    jacobi.w = MaypleMath::Sqrt(1.0f - MaypleMath::Sq(jacobi[k]));
