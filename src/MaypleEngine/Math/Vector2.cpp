...
{
  ErrorIf(index > 1, "MaypleMath::Vector2 - Subscript out of range.");
  return array[index];
}
...
void Vector2::operator/=(float rhs)
{
  ErrorIf(MaypleMath::DebugIsZero(rhs), "MaypleMath::Vector2 - Division by zero.");
  x /= rhs;
  y /= rhs;
}
...
{
  return Vector2(MaypleMath::Abs(vec.x), MaypleMath::Abs(vec.y));
}

Vector2 Min(Vec2Param lhs, Vec2Param rhs)
{
  return Vector2(MaypleMath::Min(lhs.x, rhs.x),
                 MaypleMath::Min(lhs.y, rhs.y));
}

Vector2 Max(Vec2Param lhs, Vec2Param rhs)
{
  return Vector2(MaypleMath::Max(lhs.x, rhs.x),
...
void Clamp(Vec2Ptr vec, float min, float max)
{
  ...
  MaypleMath::Clamp(vec->x, min, max);
  MaypleMath::Clamp(vec->y, min, max);
}
