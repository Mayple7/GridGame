...
void Vector3::Floor()
{
  x = MaypleMath::Floor(x);
  y = MaypleMath::Floor(y);
  z = MaypleMath::Floor(z);
}
...
{
  x = (x > 0) ? MaypleMath::Ceil(x) : MaypleMath::Floor(x);
  y = (y > 0) ? MaypleMath::Ceil(y) : MaypleMath::Floor(y);
  z = (z > 0) ? MaypleMath::Ceil(z) : MaypleMath::Floor(z);
}

void Vector3::Round()
{
  x = MaypleMath::Round(x);
  y = MaypleMath::Round(y);
  z = MaypleMath::Round(z);
}
...
Vector3 Vector3::VectorInDegrees(void)
{
  Vector3 ret;
  ret.x = MaypleMath::RadToDeg(x);
  ret.y = MaypleMath::RadToDeg(y);
  ret.z = MaypleMath::RadToDeg(z);

  return ret;
}

Vector3 Vector3::VectorInRadians(void)
{
  Vector3 ret;
  ret.x = MaypleMath::DegToRad(x);
  ret.y = MaypleMath::DegToRad(y);
  ret.z = MaypleMath::DegToRad(z);

  return ret;
}

Vector4 Vector3::ToVector4Point(void)
{
  return Vector4(x, y, z, 1.f);
}

Vector4 Vector3::ToVector4Vector(void)
{
...
{
  return Vector3(MaypleMath::Abs(vec.x), MaypleMath::Abs(vec.y), MaypleMath::Abs(vec.z));
}

Vector3 Min(Vec3Param lhs, Vec3Param rhs)
{
  return Vector3(MaypleMath::Min(lhs.x, rhs.x),
...
{
  return Vector3(MaypleMath::Max(lhs.x, rhs.x),
...
{
  ErrorIf(vec == NULL, "Vector3 - Null pointer passed for vector.");
  vec->x = MaypleMath::Clamp(vec->x, min, max);
  vec->y = MaypleMath::Clamp(vec->y, min, max);
  vec->z = MaypleMath::Clamp(vec->z, min, max);
}

Vector3 Clamped(Vec3Param vec, float min, float max)
{
  Vector3 results;
  results[0] = MaypleMath::Clamp(vec[0], min, max);
  results[1] = MaypleMath::Clamp(vec[1], min, max);
  results[2] = MaypleMath::Clamp(vec[2], min, max);
  return results;
}
...
Vector3 Vector3::operator/(float rhs) const
{
  ErrorIf(MaypleMath::DebugIsZero(rhs), "MaypleMath::Vector3 - Division by zero.");
  Vector3 ret = *this;
  ret /= rhs;
  return ret;
}
